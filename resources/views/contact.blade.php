...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Vrijblijvend adviesgesprek | Fitness Aannemer</title>

        <meta name="description" content="Plan een vrijblijvend adviesgesprek met Fitness Aannemer. Vertel ons over jouw project en wij nemen binnen 24 uur contact op.">
        <meta name="robots" content="index, follow">
        <link rel="canonical" href="{{ url('/vrijblijvend-adviesgesprek') }}">
        <link rel="icon" type="image/x-icon" href="{{ asset('favicon.ico') }}">

        {{-- Open Graph --}}
        <meta property="og:type" content="website">
        <meta property="og:url" content="{{ url('/vrijblijvend-adviesgesprek') }}">
        <meta property="og:title" content="Vrijblijvend adviesgesprek | Fitness Aannemer">
        <meta property="og:description" content="Plan een vrijblijvend adviesgesprek met Fitness Aannemer. Vertel ons over jouw project en wij nemen binnen 24 uur contact op.">
        ...
        <meta name="twitter:title" content="Vrijblijvend adviesgesprek | Fitness Aannemer">
        <meta name="twitter:description" content="Plan een vrijblijvend adviesgesprek met Fitness Aannemer. Vertel ons over jouw project en wij nemen binnen 24 uur contact op.">
    ...
    <body class="bg-secondary min-h-screen flex flex-col items-center justify-center">
        ...
        <div class="w-full max-w-2xl mx-auto px-6 py-16">
            ...
            <div class="text-center mb-12">
                ...
                <h1 class="text-white text-3xl lg:text-4xl font-bold leading-[1.05] mb-3">Vrijblijvend <span class="text-primary">adviesgesprek</span></h1>
                        ...
                        <div>
                            ...
                            <div class="custom-select relative" data-name="oppervlakte">
                                ...
                                <div class="custom-select-dropdown absolute left-0 right-0 top-full mt-2 bg-[#0d1117] border border-white/[0.1] rounded-xl overflow-hidden z-50 opacity-0 invisible translate-y-1 transition-all duration-200">
                                    <div class="custom-select-option px-4 py-3 text-white/70 text-sm cursor-pointer hover:bg-white/[0.06] hover:text-white transition-colors" data-value="< 250 m2">Kleiner dan 250 m&sup2;</div>
                                    ...
                                    <div class="custom-select-option px-4 py-3 text-white/70 text-sm cursor-pointer hover:bg-white/[0.06] hover:text-white transition-colors" data-value="500 - 750 m2">500 - 750 m&sup2;</div>
                                    <div class="custom-select-option px-4 py-3 text-white/70 text-sm cursor-pointer hover:bg-white/[0.06] hover:text-white transition-colors" data-value="750 - 1000 m2">750 - 1.000 m&sup2;</div>
                                    <div class="custom-select-option px-4 py-3 text-white/70 text-sm cursor-pointer hover:bg-white/[0.06] hover:text-white transition-colors" data-value="1000 - 1500 m2">1.000 - 1.500 m&sup2;</div>
                                    <div class="custom-select-option px-4 py-3 text-white/70 text-sm cursor-pointer hover:bg-white/[0.06] hover:text-white transition-colors" data-value="> 1500 m2">Groter dan 1.500 m&sup2;</div>
                                </div>
                            </div>
                        </div>

                        <div>
                            <label class="text-white text-xs font-semibold mb-2 block">In welke dienst ben je geïnteresseerd?</label>
                            <input type="hidden" name="dienst" id="dienst_value" value="">
                            <div class="grid grid-cols-2 gap-3">
                                @php
                                $diensten = [
                                    ['value' => 'Interieur ontwerp en turn key realisatie', 'icon' => 'fa-compass-drafting', 'label' => 'Interieur ontwerp en turn key'],
                                    ['value' => 'Apparatuur', 'icon' => 'fa-dumbbell', 'label' => 'Apparatuur'],
                                    ['value' => 'Interieur ontwerp', 'icon' => 'fa-pen-ruler', 'label' => 'Interieur ontwerp'],
                                    ['value' => 'Overig', 'icon' => 'fa-ellipsis', 'label' => 'Overig'],
                                ];
                                @endphp
                                @foreach($diensten as $dienst)
                                <button type="button" class="dienst-option group bg-white/[0.04] border-2 border-white/[0.08] rounded-xl p-4 text-left hover:border-primary/50 hover:bg-white/[0.07] transition-all duration-300 cursor-pointer flex items-center gap-3" data-value="{{ $dienst['value'] }}">
                                    <div class="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0 group-hover:bg-primary/20 transition-colors">
                                        <i class="fa-solid {{ $dienst['icon'] }} text-primary text-sm"></i>
                                    </div>
                                    <span class="text-white text-sm font-semibold leading-tight">{{ $dienst['label'] }}</span>
                                </button>
                                @endforeach
                            </div>
                        </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Step 2: Dienst selection
            const dienstOptions = form.querySelectorAll('.dienst-option');
            const dienstInput = form.querySelector('#dienst_value');

            dienstOptions.forEach(option => {
                option.addEventListener('click', () => {
                    dienstOptions.forEach(o => {
                        o.classList.remove('border-primary', 'bg-white/[0.07]');
                        o.classList.add('border-white/[0.08]');
                    });
                    option.classList.add('border-primary', 'bg-white/[0.07]');
                    option.classList.remove('border-white/[0.08]');
                    dienstInput.value = option.dataset.value;
                });
            });
